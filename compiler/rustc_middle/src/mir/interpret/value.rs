...
use super::{
    AllocId, AllocRange, Allocation, DefaultTag, InterpResult, Pointer, PointerArithmetic,
    Provenance,
};
...
pub enum ConstValue<'tcx> {
    ...
    /// Also, pointer provenance may not be `None`; integers should be represented as `Scalar::Int`.
...
impl<'tcx> ConstValue<'tcx> {
    ...
    #[inline(always)]
    pub fn from_scalar(s: Scalar, cx: &impl HasDataLayout) -> Self {
        ConstValue::Scalar(Scalar::from_scalar(s, cx))
    }
}
...
#[derive(HashStable)]
pub enum Scalar<Tag = DefaultTag> {
...
impl From<Scalar<AllocId>> for Scalar {
    #[inline(always)]
    fn from(s: Scalar<AllocId>) -> Scalar {
        match s {
            Scalar::Ptr(ptr) => Scalar::Ptr(ptr.into()),
...
impl Scalar<AllocId> {
    /// Scalar with DefaultTag can be converted to Scalar that definitely has an AllocId in all
    /// pointers.
    pub fn from_scalar(s: Scalar, cx: &impl HasDataLayout) -> Self {
        match s {
            Scalar::Ptr(Pointer { offset, provenance: Some(alloc_id) }) => {
                Scalar::Ptr(Pointer { offset, provenance: alloc_id })
            }
            Scalar::Ptr(Pointer { offset, provenance: None }) => Scalar::Int(
                ScalarInt::try_from_uint(offset.bytes(), cx.data_layout().pointer_size).unwrap(),
            ),
            Scalar::Int(int) => Scalar::Int(int),
        }
    }
...
pub enum ScalarMaybeUninit<Tag = DefaultTag> {
    Scalar(Scalar<Tag>),
    Uninit,
}
