...
impl From<Scalar<AllocId>> for Scalar {
    ...
    fn from(s: Scalar<AllocId>) -> Scalar {
        match s {
            Scalar::Ptr(ptr, size) => Scalar::Ptr(ptr.into(), size),
            Scalar::Int(int) => Scalar::Int(int),
        }
    }
}

impl From<Scalar> for Scalar<AllocId> {
    fn from(s: Scalar) -> Self {
        match s {
            Scalar::Ptr(Pointer { offset, provenance: Some(alloc_id) }, sz) => {
                Scalar::Ptr(Pointer { offset, provenance: alloc_id }, sz)
            }
            Scalar::Ptr(Pointer { offset, provenance: None }, sz) => {
                Scalar::Int(ScalarInt::try_from_uint(offset.bytes(), Size::from_bytes(sz)).unwrap())
            }
            Scalar::Int(int) => Scalar::Int(int),
        }
    }
...
impl<'tcx, Tag: Provenance> Scalar<Tag> {
    ...
    pub fn try_to_int(self) -> Option<ScalarInt> {
        match self {
            ...
            Scalar::Ptr(ptr, sz) => {
                if ptr.provenance.offset_is_addr() {
                    Some(
