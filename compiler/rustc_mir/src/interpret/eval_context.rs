...
use super::{
    AllocId, DefaultTag, GlobalId, Immediate, InterpResult, MPlaceTy, Machine, MemPlace,
...
/// A stack frame.
pub struct Frame<'mir, 'tcx, Tag = DefaultTag, Extra = ()> {
    ////////////////////////////////////////////////////////////////////////////////
...
pub struct LocalState<'tcx, Tag = DefaultTag> {
...
pub enum LocalValue<Tag = DefaultTag> {
    ...
    fn deallocate_local(&mut self, local: LocalValue<M::PointerTag>) -> InterpResult<'tcx> {
        if let LocalValue::Live(Operand::Indirect(MemPlace { ptr, .. })) = local {
            ...
            trace!(
                "deallocating local {:?}: {:?}",
                local,
                self.memory.dump_alloc(ptr.provenance.erase_for_fmt().unwrap())
            );
    ...
    for PlacePrinter<'a, 'mir, 'tcx, M>
{
    fn fmt(&self, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self.place {
            Place::Local { frame, local } => {
                ...
                match self.ecx.stack()[frame].locals[local].value {
                    ...
                    LocalValue::Live(Operand::Indirect(mplace)) => {
                        ...
                        allocs.extend(mplace.ptr.provenance.erase_for_fmt());
                    }
                    LocalValue::Live(Operand::Immediate(Immediate::Scalar(val))) => {
                        write!(fmt, " {:?}", val)?;
                        if let ScalarMaybeUninit::Scalar(Scalar::Ptr(ptr, _size)) = val {
                            allocs.extend(ptr.provenance.erase_for_fmt());
                        }
                    }
            ...
            Place::Ptr(mplace) => match mplace.ptr.provenance.erase_for_fmt() {
                Some(alloc_id) => write!(
                    fmt,
                    "by align({}) ref {:?}: {:?}",
                    mplace.align.bytes(),
