...
use super::{
    AllocId, Allocation, DefaultTag, InterpCx, MPlaceTy, Machine, MemoryKind, ValueVisitor,
};
...
fn intern_shallow<'rt, 'mir, 'tcx, M: CompileTimeMachine<'mir, 'tcx, const_eval::MemoryKind>>(
    ...
    leftover_allocations.extend(alloc.relocations().iter().filter_map(|&(_, alloc_id)| alloc_id));
