...
impl<'mir, 'tcx, M: Machine<'mir, 'tcx>> Memory<'mir, 'tcx, M> {
    ...
    pub fn global_base_pointer(
        ...
        Ok(M::tag_global_base_pointer(&self.extra, alloc_id, offset))
    }
    ...
    pub fn reallocate(
        ...
        let (alloc_id, offset) = self.ptr_force_alloc(ptr)?;
        if offset.bytes() != 0 {
            throw_ub_format!(
    ...
    pub fn check_ptr_access_align(
        ...
        self.check_and_deref_ptr(ptr, size, Some(align), msg, |alloc_id, _| {
    ...
    fn check_and_deref_ptr<T>(
        ...
        align: Option<Align>,
        msg: CheckInAllocMsg,
        alloc_size: impl FnOnce(AllocId, Size) -> InterpResult<'tcx, (Size, Align, T)>,
        ...
        Ok(match alloc_id {
            ...
            Some(alloc_id) => {
                let (allocation_size, alloc_align, ret_val) = alloc_size(alloc_id, offset)?;
    ...
    pub fn ptr_may_be_null(&self, ptr: Pointer<M::PointerTag>) -> bool {
        match M::ptr_get_alloc(&self, ptr) {
            (Some(alloc_id), offset) => {
...
impl<'mir, 'tcx, M: Machine<'mir, 'tcx>> Memory<'mir, 'tcx, M> {
    ...
    fn get_global_alloc(
        ...
        debug_assert!(tag == M::tag_global_base_pointer(memory_extra, id, Size::ZERO).provenance);
        Ok(alloc)
    }
    ...
    pub fn get<'a>(
        ...
        let ptr_and_alloc = self.check_and_deref_ptr(
            ...
            |alloc_id, offset| {
                ...
                Ok((alloc.size(), alloc.align, (alloc_id, offset, alloc)))
            },
        )?;
        if let Some((alloc_id, offset, alloc)) = ptr_and_alloc {
    ...
    pub fn get_mut<'a>(
        ...
        if let Some((alloc_id, offset)) = parts {
            let tcx = self.tcx;
        ...
        let reachable = {
            ...
            while let Some(id) = todo.pop() {
                if reachable.insert(id) {
                    ...
                    if let Some((_, alloc)) = self.alloc_map.get(id) {
                        todo.extend(
                            alloc.relocations().values().filter_map(|tag| tag.erase_for_fmt()),
                        );
                    }
                }
            }
...
impl<'a, 'mir, 'tcx, M: Machine<'mir, 'tcx>> std::fmt::Debug for DumpAllocs<'a, 'mir, 'tcx, M> {
    fn fmt(&self, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        fn write_allocation_track_relocs<'tcx, Tag: Provenance, Extra>(
            ...
            for alloc_id in alloc.relocations().values().filter_map(|tag| tag.erase_for_fmt()) {
...
impl<'mir, 'tcx, M: Machine<'mir, 'tcx>> Memory<'mir, 'tcx, M> {
    ...
    pub fn read_bytes(&self, ptr: Pointer<M::PointerTag>, size: Size) -> InterpResult<'tcx, &[u8]> {
    ...
    pub fn copy_repeatedly(
        ...
        let (src_alloc_id, src_offset) = match src_parts {
        ...
        let (dest_alloc_id, dest_offset) = match dest_parts {
    ...
    pub fn scalar_to_ptr(&self, scalar: Scalar<M::PointerTag>) -> Pointer<M::PointerTag> {
        match scalar.to_bits_or_ptr(self.pointer_size(), &self.tcx) {
            ...
            Ok(bits) => {
                ...
                let tag = M::provenance_from_addr(&self, addr);
                Pointer::new(tag, Size::from_bytes(addr))
            }
        }
    }

    /// Internal helper for functions that absolutely need a ptr to be relative to some allocation.
    #[inline(always)]
    pub(super) fn ptr_force_alloc(
        &self,
        ptr: Pointer<M::PointerTag>,
    ) -> InterpResult<'tcx, (AllocId, Size)> {
        let (alloc_id, offset) = M::ptr_get_alloc(self, ptr);
        if let Some(alloc_id) = alloc_id {
            return Ok((alloc_id, offset));
        }
        // This pointer is not inside an allocation.
        throw_ub!(DanglingIntPointer(offset.bytes(), CheckInAllocMsg::InboundsTest))
    }
}
