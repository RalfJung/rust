...
use super::{
    alloc_range, from_known_layout, mir_assign_valid_types, ConstValue, DefaultTag, GlobalId,
...
pub enum Immediate<Tag = DefaultTag> {
...
#[derive(Copy, Clone)]
pub struct ImmTy<'tcx, Tag = DefaultTag> {
    imm: Immediate<Tag>,
...
#[derive(Copy, Clone, PartialEq, Eq, HashStable, Hash)]
pub enum Operand<Tag = DefaultTag> {
...
pub struct OpTy<'tcx, Tag = DefaultTag> {
...
impl<'mir, 'tcx: 'mir, M: Machine<'mir, 'tcx>> InterpCx<'mir, 'tcx, M> {
    ...
    pub(crate) fn try_read_immediate(
        ...
        Ok(match self.op_try_as_mplace(src) {
            Ok(ref mplace) => {
    ...
    pub fn operand_field(
        ...
        let base = match self.op_try_as_mplace(op) {
            Ok(ref mplace) => {
    ...
    pub fn operand_index(
        ...
        if let Ok(index) = usize::try_from(index) {
            ...
            let mplace = self.op_assert_mem_place(op);
    ...
    pub fn operand_downcast(
        ...
        Ok(match self.op_try_as_mplace(op) {
    ...
    pub fn operand_projection(
        ...
        Ok(match proj_elem {
            ...
            Subslice { .. } | ConstantIndex { .. } | Index(_) => {
                ...
                let mplace = self.op_assert_mem_place(base);
